...
function thermal!(EP::Model, inputs::Dict, setup::Dict)
    ...
    if setup["CRM_multihours"] > 0
        ...
        @expression(EP,
            eCapResMarBalanceMultiThermal[res = 1:NCRM_multi, t in selected_hours[res]],
            sum(derating_factor(gen[y], tag = res) * EP[:eTotalCap][y] for y in THERM_ALL)
        )
        ...
        if !isempty(intersect(MAINT, THERM_COMMIT))
            thermal_maintenance_capacity_reserve_margin_multihours_adjustment!(EP, inputs)
        end
        if !isempty(intersect(FUSION, THERM_COMMIT))
            fusion_capacity_reserve_margin_multihours_adjustment!(EP, inputs)
        end
    end
