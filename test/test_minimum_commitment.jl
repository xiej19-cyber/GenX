...
@testset "Zonal minimum commitment constraint" begin
    gen = [
        ...
        minimum_commitment_test_resource("gas_1", 2, 1, 200.0;
    ...
    @test !haskey(JuMP.object_dictionary(model), :cMinimumCommitmentGas)
    ...
    mktempdir() do case_path
        ...
        gen = [
            ...
            minimum_commitment_test_resource("gas_1", 3, 2, 100.0;
                fuel = "naturalgas_test"),
            minimum_commitment_test_resource("gas_2", 4, 1, 100.0;
                eligible = 0, fuel = "naturalgas_test"),
        ]
        inputs = Dict{String, Any}(
            "T" => 3,
            "Z" => 2,
            "RESOURCES" => gen,
            "THERM_COMMIT" => [1, 2, 3, 4],
        )
        ...
        CSV.write(joinpath(system_path, "Minimum_commitment.csv"),
            DataFrame(Time_Index = 1:3, Zone_1 = [0.0, 0.7, 0.6],
                Zone_2 = [0.5, 0.5, 0.0]))
        GenX.load_minimum_commitment!(setup, case_path, inputs)
